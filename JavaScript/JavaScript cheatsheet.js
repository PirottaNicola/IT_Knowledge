...
// Throws an error when attempting to delete undeletable properties.
// Prohibits the use of octal literals.
// Throws an error on duplicate property names in objects.
// Makes eval() a local scope, preventing it from modifying the outer scope.
// Throws an error when attempting to write to a read-only property.
// Throws an error on the use of with statements.
// In strict mode, this is undefined in functions that are not called as methods.
// Using strict mode can help you avoid some common JavaScript bugs and make your code more reliable and predictable.
...
console.log(tommy, restOfFamily) //tommy è il primo elemento, il resto è in restOfFamily
...
for (const [i, giorno] of settimana.entries()) {
...
class Person2 {
  ...
  calcAge(currentYear) {
    //viene aggiunto nel prototype, non nell'object --> buone performance
